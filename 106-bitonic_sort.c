 ...
 * @size: array length
 */
void bitonic_sort(int arr[], int low, int nelemnt, int order, int size)
{
    int mid;
    ...
    if (nelemnt > 1)
    {
        ...
        mid = nelemnt / 2;
        bitonic_sort(arr, low, mid, 1, size);
        bitonic_sort(arr, low + mid, mid, 0, size);
...
/**
 * bitonic_sort_prep - prepare the terrain to bitonic sort algorithm
 * @array: array
 * @size: array length
 */
void bitonic_sort_prep(int *array, size_t size)
{
    int order = 1;
    if (!array || size < 2)
        return;
    bitonic_sort(array, 0, size, order, size);
}
